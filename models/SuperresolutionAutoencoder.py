...
class VGG16EncoderWithSkipConnections(nn.Module):
    def __init__(self, input_channels=3):
        ...
        self.block1 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)  # Downsample to 128x128 (1/2)
        )
        ...
        self.block2 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)  # Downsample to 64x64 (1/4)
        )
        ...
        self.block3 = nn.Sequential(
            ...
            nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)  # Downsample to 32x32 (1/8)
        )
        ...
        self.block4 = nn.Sequential(
            ...
            nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)  # Downsample to 16x16 (1/16)
        )
        ...
        self.block5 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)  # Downsample to 8x8  (1/32)
        )
...
class VGG16Decoder(nn.Module):
    def __init__(self):
        ...
        self.block5 = nn.Sequential(
            ...
            nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1),  # Upsample to 8x8 (1/16)
            nn.ReLU(inplace=True)
        )
        ...
        # Block 3
        self.block3 = nn.Sequential(
            nn.Conv2d(256 + 256, 256, kernel_size=3, stride=1, padding=1),  # Concatenate channels
            nn.ReLU(inplace=True),
            nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True),
        ...
        # Block 2 - Todo możliwa redukcja jednej warstwy ()
        self.block2 = nn.Sequential(
